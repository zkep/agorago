...
func GenerateDocs(curpath string) {
	...
	if err != nil {
		log.Fatalf("Error while parsing router.go: %s", err)
	}
		...
		for _, c := range f.Comments {
			for _, s := range strings.Split(c.Text(), "\n") {
				if strings.HasPrefix(s, "@APIVersion") {
					...
					if len(p) < 2 {
						log.Fatalf("Not enough params for security: %d\n", len(p))
					}
					out.Type = p[1]
					switch out.Type {
					case "oauth2":
						if len(p) < 6 {
							log.Fatalf("Not enough params for oauth2: %d\n", len(p))
						}
						if !(p[3] == "implicit" || p[3] == "password" || p[3] == "application" || p[3] == "accessCode") {
							log.Fatalf("Unknown flow type: %s. Possible values are `implicit`, `password`, `application` or `accessCode`.\n", p[1])
						}
					...
					case "apiKey":
						if len(p) < 4 {
							log.Fatalf("Not enough params for apiKey: %d\n", len(p))
						}
						if !(p[3] == "header" || p[3] == "query") {
							log.Fatalf("Unknown in type: %s. Possible values are `query` or `header`.\n", p[4])
						}
						out.Name = p[2]
						out.In = p[3]
						if len(p) > 4 {
					...
					default:
						log.Fatalf("Unknown security type: %s. Possible values are `oauth2`, `apiKey` or `basic`.\n", p[1])
					}
...
func analyseNSInclude(baseurl string, ce *ast.CallExpr) string {
	...
	for _, p := range ce.Args {
		...
		if _, ok := p1.(*ast.UnaryExpr); ok {
			...
			log.Printf("Couldn't determine type\n")
			continue
		}
...
func analyseControllerPkg(vendorPath, localName, pkgpath string) {
	...
	gopaths := GetGOPATHs()
	if len(gopaths) == 0 {
		log.Fatalf("GOPATH environment variable is not set or empty")
	}
	pkgRealpath := ""
	...
	if pkgRealpath != "" {
		...
		log.Fatalf("Package '%s' does not exist in the GOPATH or vendor path", pkgpath)
	}
	...
	if err != nil {
		log.Fatalf("Error while parsing dir at '%s': %s", pkgpath, err)
	}
...
func isSystemPackage(pkgpath string) bool {
	...
	if goroot == "" {
		log.Fatalf("GOROOT environment variable is not set or empty")
	}
		...
		for _, c := range comments.List {
			...
			if strings.HasPrefix(t, "@router") {
				...
				if respType == "{object}" || respType == "{array}" {
					...
					if schemaName == "" {
						log.Fatalf("[%s.%s] Schema must follow {object} or {array}", controllerName, funcName)
					}
				...
				if len(p) < 4 {
					log.Fatalf(controllerName + "_" + funcName + "'s comments @Param should have at least 4 params")
				}
				...
				case "body":
					break
				default:
					log.Printf("[%s.%s] Unknown param location: %s. Possible values are `query`, `header`, `path`, `formData` or `body`.\n", controllerName, funcName, p[1])
				}
				para.In = p[1]
...
L:
	...
	if m.Title == "" {
		...
		if _, found := rootapi.Definitions[str]; !found {
			log.Printf("Cannot find the object: %s", str)
		}
		m.Title = objectname
		...
		for _, fl := range pkg.Files {
			for _, obj := range fl.Scope.Objects {
				if obj.Kind == ast.Con {
					...
					if !ok {
						log.Fatalf("Unknown type without ValueSpec: %v", vs)
					}
					...
					for i, val := range vs.Values {
						...
						if !ok {
							log.Printf("Unknown type without BasicLit: %v", v)
							continue
						}
						...
						case token.INT:
							...
							if err != nil {
								log.Printf("Unknown type with BasicLit to int: %v", v.Value)
								continue
							}
			...
			if field.Names != nil {
				...
				if defaultValue != "" {
					...
					if r.MatchString(defaultValue) {
						...
						log.Printf("Invalid default value: %s", defaultValue)
					}
				}
...
func getSecurity(t string) (security map[string][]string) {
	...
	if len(p) == 0 {
		log.Fatalf("No params for security specified\n")
	}
...
func str2RealType(s string, typ string) interface{} {
	...
	if err != nil {
		log.Printf("Invalid default value type '%s': %s", typ, s)
		return s
	}
